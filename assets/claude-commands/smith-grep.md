description: Smith-style targeted grep for Azure DevOps and GitHub repos via smith CLI (read-only)
---
...
Output contract:
- Return concise findings with source paths in `project/repository:path` or `org/repository:path` format.
