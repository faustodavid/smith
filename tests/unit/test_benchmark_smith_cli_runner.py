...
class _FakeClient:
    ...
    def execute_discover_projects(self, *, provider: str) -> list[dict[str, Any]]:
    ...
    def execute_code_search(
        ...
        *,
        ...
        del provider, query, project, repos, skip, take
        self.search_calls += 1
...
def test_in_process_runner_caches_successful_normalized_commands(monkeypatch: Any) -> None:
    ...
    runner = InProcessSmithCliRunner()
    first = runner.execute('smith --github-org openai code search "webhook"')
    second = runner.execute('code search "webhook" --provider github')
